/*
  Parts based on the work of Zach Archer http://zacharcher.com/
...
#include "Charliplexing8x7.h"      //initializes the LoL Sheild library
...
float phase = 0.00;
float colorStretch = 0.25f;    // Higher numbers will produce tighter color bands. I like 0.11 .
float phaseIncrement =.025;    // Bigger numbers are faster
...
void loop() {
  ...
  // We want values that fit the LED grid: x values between 0..7, y values between 0..8 .
  // The sin() function returns values in the range of -1.0..1.0, so scale these to our desired ranges.
  // The phase value is multiplied by various constants; I chose these semi-randomly, to produce a nice motion.
  //Point p1 = { (sin(phase*1.000)+1.0) * 3, (sin(phase*1.310)+1.0) * 3.0 };
  //Point p2 = { (sin(phase*1.770)+1.0) * 3, (sin(phase*2.865)+1.0) * 3.0 };
  Point p1 = { (sin(phase*1.000)+1.0) * 3.5, (sin(phase*1.310)+1.0) * 3 };
  Point p2 = { (sin(phase*1.770)+1.0) * 3.5, (sin(phase*2.865)+1.0) * 3};
  ...
  for( row=0; row<7; row++ ) {
    ...
    for( col=0; col<8; col++ ) {
      ...
      float color_f = (cos( distance * colorStretch ) + 1.0) * 0.2;  // range: 0.0...1.0
