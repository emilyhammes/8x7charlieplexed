/*
  ...
  Modified by Emily Hammes 13/Feb/18 using code written by Zach Archer http://zacharcher.com/ 
  which was written for use with the LOL shield
...
const float phaseIncrement = .01;     // How fast the sines move. I like 0.1 .
...
void loop() {
  ...
  for( col=0; col<8; col++ ) {
    ...
    int lightSine = int( round( lightSineThisColumn*3 ) ) + 3;
    ...
    int darkSine = int( round( sin(phase+drift+float(col)*stretch)*3 ) ) + 3;
